...
struct DiagramRenderer {
    ...
    /// Resolves a concept's display term using the preferred term from the server
    /// (via displayNameMap) if available, falling back to the normalForm's embedded term.
    ///
    /// This ensures diagrams show current preferred terms (e.g., "mg" instead of
    /// "milligram", "mL" instead of "Milliliter") rather than the terms authored
    /// into the normalForm.
    private static func resolvedTerm(for concept: SNOMEDConceptReference, data: VisualizationData) -> String {
        data.displayName(for: concept.conceptId) ?? concept.term ?? concept.conceptId
    }

    /// Attempts to build a human-readable display term from a nested postcoordinated
    /// expression.
    ///
    /// Handles composite units like `(415777001|Unit of mass concentration|:
    /// {700000071000036103|Has denominator units|=258773002|mL|},
    /// {700000091000036104|Has numerator units|=258684004|mg|})` → "mg/mL".
    ///
    /// Known patterns:
    /// - Numerator + denominator → "numerator/denominator"
    ///
    /// - Returns: A composed term if a known pattern is matched, nil otherwise.
    private static func resolvedExpressionTerm(for expr: SNOMEDExpression, data: VisualizationData) -> String? {
        guard let refinement = expr.refinement else { return nil }

        // Known attribute type IDs for composite unit patterns
        let numeratorIds: Set<String> = [
            "700000091000036104",  // Has numerator units (AU)
            "733725009"            // Has numerator units (INT)
        ]
        let denominatorIds: Set<String> = [
            "700000071000036103",  // Has denominator units (AU)
            "733722007"            // Has denominator units (INT)
        ]

        let allAttributes = refinement.attributeGroups.flatMap(\.attributes)
            + refinement.ungroupedAttributes

        var numeratorTerm: String?
        var denominatorTerm: String?

        for attr in allAttributes {
            let attrId = attr.name.conceptId
            if numeratorIds.contains(attrId), case .conceptReference(let ref) = attr.value {
                numeratorTerm = resolvedTerm(for: ref, data: data)
            } else if denominatorIds.contains(attrId), case .conceptReference(let ref) = attr.value {
                denominatorTerm = resolvedTerm(for: ref, data: data)
            }
        }

        if let num = numeratorTerm, let denom = denominatorTerm {
            return "\(num)/\(denom)"
        }
    ...
    private static func generateFromNormalForm(_ normalForm: String, data: VisualizationData) -> String {
        ...
        do {
            var parser = try SNOMEDExpressionParser(input: cleaned)
            expression = try parser.parse()

            // Log parsed expression structure
            AppLog.debug(AppLog.general, "Parsed normal form for \(data.concept.conceptId): focusConcepts=\(expression.focusConcepts.map { $0.conceptId }.joined(separator: ", "))")
            if let ref = expression.refinement {
                AppLog.debug(AppLog.general, "Normal form refinement: groups=\(ref.attributeGroups.count) ungrouped=\(ref.ungroupedAttributes.count)")
            } else {
                AppLog.debug(AppLog.general, "Normal form has no refinement")
            }
            ...
            if expression.focusConcepts.count == 1 &&
               expression.focusConcepts[0].conceptId == data.concept.conceptId {
                AppLog.debug(AppLog.general, "Normal form has concept \(data.concept.conceptId) as its own parent — looking for parent property")
                ...
                if !parentProps.isEmpty {
                    ...
                    for parentProp in parentProps {
                        ...
                        default:
                            AppLog.warning(AppLog.general, "Unexpected parent property value type for \(data.concept.conceptId)")
                        }
                    }

                    guard !allParentIds.isEmpty else {
                        AppLog.warning(AppLog.general, "No parent IDs extracted from \(parentProps.count) parent properties for \(data.concept.conceptId)")
                        return generateSimpleSNOMEDDiagram(data)
                    }

                    AppLog.debug(AppLog.general, "Found \(parentProps.count) parent properties with \(allParentIds.count) parent ID(s): \(allParentIds.joined(separator: ", "))")
                    ...
                    AppLog.debug(AppLog.general, "No parent property found for \(data.concept.conceptId) — falling back to simple diagram")
                    return generateSimpleSNOMEDDiagram(data)
                }
            }
        } catch {
            AppLog.warning(AppLog.general, "Failed to parse normal form: \(error) input=\(String(cleaned.prefix(200)))")
    ...
    private static func generateDiagramSVG(_ expression: SNOMEDExpression, conceptDisplay: String, data: VisualizationData) -> String {
        ...
        if !expression.focusConcepts.isEmpty {
            ...
            for (index, parentRef) in expression.focusConcepts.enumerated() {
                ...
                let parentTerm = resolvedTerm(for: parentRef, data: data)
                // Calculate width based on actual text content
    ...
    private static func drawAttribute(_ attribute: SNOMEDAttribute, x: Int, y: Int, svgElements: inout [String], data: VisualizationData) -> (maxX: Int, height: Int) {
        ...
        let attrTerm = resolvedTerm(for: attribute.name, data: data)
        ...
        case .conceptReference(let ref):
            ...
            let valueTerm = resolvedTerm(for: ref, data: data)
        ...
        case .expression(let expr):
            // Try to build a human-readable term from the nested expression.
            // For composite units (e.g., mg/mL), resolve from numerator/denominator
            // attributes. Otherwise fall back to the focus concept's preferred term.
            if let focusConcept = expr.focusConcepts.first {
                let valueTerm = resolvedExpressionTerm(for: expr, data: data)
                    ?? resolvedTerm(for: focusConcept, data: data)
                let maxValueWidth = 400  // Reasonable max width
    ...
    private static func generateFullDiagramHTML(diagramSVG: String, attributesHTML: String, hasAttributes: Bool, conceptId: String, conceptTerm: String?) -> String {
        // Safely encode concept info as JSON for JavaScript consumption.
        // Using JSONSerialization avoids XSS via string interpolation into script blocks.
        let conceptData: [String: String] = [
            "conceptId": conceptId,
            "conceptTerm": conceptTerm ?? ""
        ]
        let jsonData = (try? JSONSerialization.data(withJSONObject: conceptData)) ?? Data()
        let jsonString = String(data: jsonData, encoding: .utf8) ?? #"{"conceptId":"","conceptTerm":""}"#
        ...
        <body>
            ...
            <script type="application/json" id="concept-data">\(jsonString)</script>
            <script>
                // Concept info for filename generation (parsed from JSON to prevent injection)
                const _conceptData = JSON.parse(document.getElementById('concept-data').textContent);
                const conceptId = _conceptData.conceptId;
                const conceptTerm = _conceptData.conceptTerm;
