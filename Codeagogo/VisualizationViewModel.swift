            ...
            for conceptId in conceptIds {
                group.addTask {
                    do {
                        ...
                        // For display name, we need to use the regular lookup which returns ConceptResult
                        // This is more reliable than trying to extract from properties
                        var displayName: String?
                        do {
                            let result = try await self.client.lookup(conceptId: conceptId)
                            displayName = result.pt ?? result.fsn
