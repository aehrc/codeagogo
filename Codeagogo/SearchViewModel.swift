...
final class SearchViewModel: ObservableObject {
    ...
    ///
    /// The search uses either SNOMED CT (with optional edition filter) or
    /// a configured non-SNOMED code system, depending on the user's selection.
    func search() {
        ...
        currentSearchTask = Task {
            do {
                ...
                if settings.isSNOMEDSelected {
                    // SNOMED CT search with optional edition filter
                    searchResults = try await client.searchConcepts(
                        filter: trimmed,
                        editionURI: settings.selectedEditionURI
                    )
                } else if let codeSystemURI = settings.selectedCodeSystemURI {
                    // Non-SNOMED code system search
                    if let ontoClient = client as? OntoserverClient {
                        searchResults = try await ontoClient.searchInCodeSystem(
                            filter: trimmed,
                            system: codeSystemURI
                        )
                    } else {
                        // Fallback for testing - use SNOMED search
                        searchResults = try await client.searchConcepts(
                            filter: trimmed,
                            editionURI: nil
                        )
    ...
    /// Sets up debounced search triggered by searchText, edition, or code system changes.
    private func setupSearchDebouncing() {
        ...
        // Re-search when the code system selection changes
        settings.$selectedCodeSystemURI
            .dropFirst()  // Skip initial value
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in
                self?.search()
