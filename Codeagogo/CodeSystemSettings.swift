...
final class CodeSystemSettings: ObservableObject {
    ...
    /// Validates that a URI has a valid structure for a code system identifier.
    ///
    /// Checks that the URI has an http or https scheme, a non-empty host,
    /// and no query parameters or fragments.
    ///
    /// - Parameter uri: The URI string to validate.
    /// - Returns: `true` if the URI is structurally valid.
    static func isValidCodeSystemURI(_ uri: String) -> Bool {
    ...
    ///
    /// If a system with the same URI already exists, or the URI is not a valid
    /// code system identifier, this method does nothing.
    ///
    /// - Parameters:
    ...
    func addSystem(uri: String, title: String) {
        ...
        guard Self.isValidCodeSystemURI(uri) else {
            AppLog.warning(AppLog.general, "Rejected invalid code system URI: \(uri)")
            return
        }
