...
struct ConceptResult {
    /// The concept code (SNOMED CT ID or other code system identifier).
    let conceptId: String

    /// Human-readable edition/version name.
    ///
    /// For SNOMED CT: "International (20240101)", "Australian (20231130)"
    /// For other systems: May be empty or contain version info
    let branch: String

    /// The Fully Specified Name — the unambiguous term with semantic tag.
    ///
    /// Example: "Diabetes mellitus (disorder)"
    /// For non-SNOMED systems, this may be nil.
    let fsn: String?

    /// The Preferred Term / Display name.
    ///
    /// Example: "Diabetes mellitus"
    let pt: String?

    /// Whether the concept is currently active.
    ///
    ...
    /// The module that contains this concept (SNOMED CT specific).
    ///
    /// Example: "900000000000207008" (International Core Module)
    let moduleId: String?

    /// The code system URI, or nil for SNOMED CT.
    ///
    /// Examples:
    /// - `nil` or `"http://snomed.info/sct"` for SNOMED CT
    /// - `"http://loinc.org"` for LOINC
    /// - `"http://hl7.org/fhir/sid/icd-10-cm"` for ICD-10-CM
    let system: String?
    ...
    ///
    /// - Returns: A display name like "SNOMED CT", "LOINC", etc.
    var systemName: String {
        guard let system = system else { return "SNOMED CT" }
        switch system {
        case "http://snomed.info/sct", "http://snomed.info/xsct":
            return "SNOMED CT"
        case "http://loinc.org":
            return "LOINC"
        ...
        case "http://hl7.org/fhir/sid/icd-9-cm":
            return "ICD-9-CM"
        default:
            // Return last path component or full URI
            if let lastComponent = system.split(separator: "/").last {
    ...
    var isSNOMEDCT: Bool {
        guard let system = system else { return true }
        return system.contains("snomed.info")
    }

    /// Creates a ConceptResult with default system (SNOMED CT).
    init(conceptId: String, branch: String, fsn: String?, pt: String?,
         active: Bool?, effectiveTime: String?, moduleId: String?, system: String? = nil) {
...
/// Protocol abstracting URLSession for dependency injection and testability.
///
/// Allows tests to provide a mock URL session without hitting the network.
protocol URLSessionProtocol: Sendable {
    /// Performs a data request and returns the response.
    func data(for request: URLRequest) async throws -> (Data, URLResponse)
}

/// Default conformance so `URLSession.shared` can be used directly.
...
final class OntoserverClient: ConceptSearching, @unchecked Sendable {
    ...
    /// Cached SNOMED editions shared across instances, with a 30-minute TTL.
    private static var cachedEditions: [SNOMEDEdition]?
    private static var editionsCacheTime: Date?
    private static let editionsCacheTTL: TimeInterval = 30 * 60  // 30 minutes

    /// User-Agent header value for identifying this application in server logs.
    ///
    /// Includes an anonymous install ID to enable unique-install counting
    /// from server logs without collecting personal data.
    private var userAgent: String {
        let appName = Bundle.main.infoDictionary?["CFBundleName"] as? String ?? "Codeagogo"
        let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "unknown"
        let installId = InstallMetrics.currentInstallId
        return "\(appName)/\(appVersion) (macOS; \(installId))"
    }
    ...
    func lookup(conceptId: String) async throws -> ConceptResult {
        ...
        let editions = try await getCachedOrFetchEditions()
    ...
    struct BatchLookupResult {
        ...
        /// Map from concept ID to its active status (from cache).
        ///
        /// Only populated for concepts that were found in cache.
        /// `true` = active, `false` = inactive, missing = unknown.
        let activeByCode: [String: Bool]
        ...
        /// Returns whether a code is active, or nil if unknown.
        func isActive(for code: String) -> Bool? { activeByCode[code] }
    }
    ...
    func batchLookup(conceptIds: [String]) async throws -> BatchLookupResult {
        guard !conceptIds.isEmpty else {
            return BatchLookupResult(ptByCode: [:], fsnByCode: [:], activeByCode: [:])
        }
        ...
        for conceptId in conceptIds {
            if let cached = await cache.get(conceptId, ttl: NetworkConstants.cacheTTL) {
                ...
                if let active = cached.active { activeByCode[conceptId] = active }
            } else {
        ...
        let requestBody = buildBatchLookupRequestBody(conceptIds: uncachedIds, editions: [])
        ...
        mergeBatchResults(fetchedResults, into: &ptByCode, fsnByCode: &fsnByCode, activeByCode: &activeByCode)
        ...
        let notFoundIds = uncachedIds.filter { code in
            ptByCode[code] == nil && !SCTIDValidator.isCoreSCTID(code)
        }
        if !notFoundIds.isEmpty {
            AppLog.info(AppLog.network, "batchLookup \(notFoundIds.count) namespaced codes not in default edition, trying other editions")
            let editions = (try? await getCachedOrFetchEditions()) ?? []
            if !editions.isEmpty {
                let editionResults = await withTaskGroup(of: (String, ConceptResult?).self) { group in
                    for conceptId in notFoundIds {
                        group.addTask {
                            let result = try? await self.lookupInAllEditions(conceptId: conceptId, editions: editions)
                ...
                for (conceptId, result) in editionResults {
                    guard let result else { continue }
                    if let pt = result.pt { ptByCode[conceptId] = pt }
                    if let fsn = result.fsn { fsnByCode[conceptId] = fsn }
                    if let active = result.active { activeByCode[conceptId] = active }
                    await cache.set(conceptId, result: result)
                }
            }
        }

        AppLog.info(AppLog.network, "batchLookup found \(ptByCode.count) concepts total")
        return BatchLookupResult(ptByCode: ptByCode, fsnByCode: fsnByCode, activeByCode: activeByCode)
    }

    /// Merges batch lookup results into the running totals and caches them.
    private func mergeBatchResults(
        _ results: BatchLookupResult,
        into ptByCode: inout [String: String],
        fsnByCode: inout [String: String],
        activeByCode: inout [String: Bool]
    ) {
        for (code, pt) in results.ptByCode {
            ptByCode[code] = pt
            let fsn = results.fsnByCode[code]
            let active = results.activeByCode[code]
            ...
            Task { await cache.set(code, result: cachedResult) }
        }
        for (code, fsn) in results.fsnByCode {
            fsnByCode[code] = fsn
        }
        for (code, active) in results.activeByCode {
            activeByCode[code] = active
        }
    }

    /// Builds the FHIR Parameters request body for batch lookup via ValueSet/$expand.
    private func buildBatchLookupRequestBody(conceptIds: [String], editions: [SNOMEDEdition]) -> [String: Any] {
        let concepts: [[String: String]] = conceptIds.map { ["code": $0] }

        // If no specific editions requested, use the bare system URI (server default — fast path)
    ...
    private func parseBatchLookupResults(from response: ValueSetExpansionResponse) -> BatchLookupResult {
        ...
        guard let contains = response.expansion?.contains else {
            return BatchLookupResult(ptByCode: ptByCode, fsnByCode: fsnByCode, activeByCode: [:])
        }
        ...
        for item in contains {
            ...
            // Extract active status from property extension
            ...
            //                 { "url": "value", "valueBoolean": true/false }] }
            if let extensions = item.extension {
                for ext in extensions {
                    if ext.url == "http://hl7.org/fhir/5.0/StructureDefinition/extension-ValueSet.expansion.contains.property",
                       let parts = ext.extension {
                        var isInactiveProperty = false
                        var inactiveValue: Bool?

                        for part in parts {
                            if part.url == "code" && part.valueCode == "inactive" {
                                isInactiveProperty = true
                            }
                            if part.url == "value", let value = part.valueBoolean {
                                inactiveValue = value
                            }
                        }

                        if isInactiveProperty, let inactive = inactiveValue {
                            // inactive=true means active=false
                            activeByCode[code] = !inactive
                        }
                    }
                }
            }
        ...
        AppLog.info(AppLog.network, "batchLookup found \(ptByCode.count) concepts, \(activeByCode.count) with active status")
        return BatchLookupResult(ptByCode: ptByCode, fsnByCode: fsnByCode, activeByCode: activeByCode)
    }
    ...
    func searchConcepts(filter: String, editionURI: String?, count: Int = 30) async throws -> [SearchResult] {
        ...
        let allEditions = try await getCachedOrFetchEditions()
    ...
    /// - Throws: Network or parsing errors
    func getAvailableEditions() async throws -> [SNOMEDEdition] {
        try await getCachedOrFetchEditions()
    }
    ...
    ///
    /// Uses `ValueSet/$expand` with the ECL expression encoded in the implicit
    /// ValueSet URL (`http://snomed.info/sct?fhir_vs=ecl/<expression>`).
    ///
    /// - Parameters:
    ///   - expression: The ECL expression to evaluate
    ///   - count: Maximum number of concepts to return
    /// - Returns: An ``ECLEvaluationResult`` with the total count and matching concepts
    /// - Throws: Network or parsing errors
    func evaluateECL(expression: String, count: Int = 50) async throws -> ECLEvaluationResult {
        let trimmed = expression.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else {
            return ECLEvaluationResult(total: 0, concepts: [])
        }

        AppLog.info(AppLog.network, "evaluateECL expression=\(trimmed) count=\(count)")

        // URL-encode the ECL expression for the implicit ValueSet URL
        guard let encodedExpression = trimmed.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else {
            throw URLError(.badURL)
        }

        let implicitValueSetURL = "http://snomed.info/sct?fhir_vs=ecl/" + encodedExpression

        // Build FHIR Parameters body
        let requestBody: [String: Any] = [
            "resourceType": "Parameters",
            "parameter": [
                ["name": "url", "valueUri": implicitValueSetURL],
                ["name": "count", "valueInteger": count],
                ["name": "includeDesignations", "valueBoolean": true]
            ]
        ]

        AppLog.debug(AppLog.network, "evaluateECL implicitVS=\(implicitValueSetURL)")

        let url = baseURL.appendingPathComponent("ValueSet/$expand")
        let response: ValueSetExpansionResponse = try await postJSON(url, body: requestBody)

        let total = response.expansion?.total ?? 0
        let concepts = (response.expansion?.contains ?? []).compactMap { entry -> ECLEvaluationConcept? in
            guard let code = entry.code, let display = entry.display else {
                return nil
            }
            // Extract FSN from designations (use code 900000000000003001)
            let fsn = entry.designation?.first(where: {
                $0.use?.code == "900000000000003001"
            })?.value
            return ECLEvaluationConcept(code: code, display: display, fsn: fsn)
        }

        AppLog.debug(AppLog.network, "evaluateECL total=\(total) returned=\(concepts.count)")

        return ECLEvaluationResult(total: total, concepts: concepts)
    }

    // MARK: - Multi-Code-System Support

    /// Fetches all code systems available on the configured FHIR server.
    ///
    /// Uses `GET /CodeSystem?_summary=true&_count=200` to retrieve metadata
    /// for all code systems. Excludes SNOMED CT editions (handled separately).
    ///
    /// - Returns: Array of available code systems
    /// - Throws: Network or parsing errors
    func getAvailableCodeSystems() async throws -> [AvailableCodeSystem] {
        guard var comps = URLComponents(url: baseURL.appendingPathComponent("CodeSystem"), resolvingAgainstBaseURL: false) else {
            throw OntoserverError.invalidURL("Cannot create URL components from base URL")
        }
        comps.queryItems = [
            URLQueryItem(name: "_summary", value: "true"),
            URLQueryItem(name: "_count", value: "200"),
            URLQueryItem(name: "_format", value: "json")
        ]

        guard let url = comps.url else {
            throw OntoserverError.invalidURL("Invalid query parameters for code systems request")
        }

        AppLog.debug(AppLog.network, "getAvailableCodeSystems request url=\(url.absoluteString)")

        let bundle: FHIRBundle = try await getJSON(url)

        guard let entries = bundle.entry else {
            AppLog.info(AppLog.network, "no code systems found")
            return []
        }

        // Filter out SNOMED CT editions and build list
        var systems: [AvailableCodeSystem] = []
        var seenUrls = Set<String>()

        for entry in entries {
            guard let codeSystem = entry.resource,
                  let url = codeSystem.url else { continue }

            // Skip SNOMED CT (handled separately)
            if url.contains("snomed.info") { continue }

            // Deduplicate by URL
            if seenUrls.contains(url) { continue }
            seenUrls.insert(url)

            let title = codeSystem.title ?? codeSystem.name ?? url
            systems.append(AvailableCodeSystem(
                url: url,
                title: title,
        ...
        systems.sort { $0.title.localizedCaseInsensitiveCompare($1.title) == .orderedAscending }

        AppLog.info(AppLog.network, "found \(systems.count) non-SNOMED code systems")
        return systems
    }

    /// Looks up a code in a specific code system.
    ///
    /// Uses `CodeSystem/$lookup` with the specified system URI.
    ///
    /// - Parameters:
    ///   - code: The code to look up
    ///   - system: The code system URI (e.g., "http://loinc.org")
    /// - Returns: The concept result, or nil if not found
    /// - Throws: Network errors (404 returns nil instead of throwing)
    func lookupInCodeSystem(code: String, system: String) async throws -> ConceptResult? {
        guard var comps = URLComponents(url: baseURL.appendingPathComponent("CodeSystem/$lookup"), resolvingAgainstBaseURL: false) else {
            throw OntoserverError.invalidURL("Cannot create URL components from base URL")
        }
        comps.queryItems = [
        ...
        guard let url = comps.url else {
            throw OntoserverError.invalidURL("Invalid query parameters for code system lookup")
        }

        AppLog.debug(AppLog.network, "lookupInCodeSystem code=\(code) system=\(system)")

        do {
            let response: FHIRParameters = try await getJSON(url)
            return parseCodeSystemLookupResult(response, code: code, system: system)
        } catch let error as NSError where error.code == 404 {
            AppLog.debug(AppLog.network, "code not found in system=\(system) code=\(code)")
            return nil
        }
    }

    /// Searches configured code systems for a code.
    ///
    /// Tries each system in parallel and returns the first match found.
    /// This is used for non-SNOMED codes that don't pass Verhoeff validation.
    ///
    /// - Parameters:
    ///   - code: The code to look up
    ///   - systems: Array of code system URIs to search
    /// - Returns: The first match found, or nil if not in any configured system
    ...
    func lookupInConfiguredSystems(code: String, systems: [String]) async throws -> ConceptResult? {
        ...
        AppLog.info(AppLog.network, "lookupInConfiguredSystems code=\(code) systems=\(systems)")

        // Try all systems in parallel
        return try await withThrowingTaskGroup(of: ConceptResult?.self) { group in
            for system in systems {
                group.addTask {
                    try await self.lookupInCodeSystem(code: code, system: system)
                }
            }
            ...
            for try await result in group {
                if let result = result {
                    group.cancelAll()
                    AppLog.info(AppLog.network, "found code=\(code) in system=\(result.system ?? "unknown")")
                    return result
                }
            }

            AppLog.debug(AppLog.network, "code not found in any configured system code=\(code)")
            return nil
        }
    }

    /// Searches for concepts in a specific code system.
    ///
    /// Uses `ValueSet/$expand` with a filter, limited to the specified code system.
    ///
    /// - Parameters:
    ///   - filter: The search text
    ///   - system: The code system URI to search
    ///   - count: Maximum number of results (default: 30)
    /// - Returns: Array of matching concepts
    /// - Throws: Network or parsing errors
    func searchInCodeSystem(filter: String, system: String, count: Int = 30) async throws -> [SearchResult] {
        guard !filter.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return []
        }

        AppLog.info(AppLog.network, "searchInCodeSystem filter=\(filter) system=\(system)")

        // Build the request body for ValueSet/$expand
        let valueSetResource: [String: Any] = [
            "resourceType": "ValueSet",
            "compose": [
                "include": [
        ...
        return parseCodeSystemSearchResults(from: response, system: system)
    }

    /// Parses lookup results for a non-SNOMED code system.
    private func parseCodeSystemLookupResult(_ params: FHIRParameters, code: String, system: String) -> ConceptResult? {
        var display: String?
        var version: String?
        ...
        guard let display = display else {
            AppLog.warning(AppLog.network, "no display found for code=\(code) system=\(system)")
            return nil
        }
        ...
        return ConceptResult(
            conceptId: code,
            branch: branch,
            fsn: nil,  // Non-SNOMED systems typically don't have FSN
            pt: display,
            active: nil,  // Status often not available for non-SNOMED
            effectiveTime: nil,
            moduleId: nil,
            system: system
        )
    ...
    /// Parses search results from a code system search.
    private func parseCodeSystemSearchResults(from response: ValueSetExpansionResponse, system: String) -> [SearchResult] {
        guard let contains = response.expansion?.contains else {
            return []
        }
        ...
        for item in contains {
            ...
            // For non-SNOMED systems, use the system URI as the "edition" name
            ...
            default:
                systemName = system.split(separator: "/").last.map(String.init) ?? system
            }
            ...
            results.append(SearchResult(
                ...
                editionName: systemName
            ))
        }

        AppLog.info(AppLog.network, "searchInCodeSystem found \(results.count) results in \(system)")
        return results
    }
    ...
    private func performPostRequest<T: Decodable>(_ url: URL, body: [String: Any]) async throws -> T {
        ...
        do {
            ...
            guard let http = resp as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
                ...
                let userMessage = Self.extractUserMessage(from: data, httpCode: code)
                throw NSError(
    ...
    ///
    /// By omitting the version parameter, the server returns the preferred term
    /// from its default edition (e.g., SCTAU for Australian servers), which gives
    /// locale-appropriate display terms.
    ///
    /// - Parameters:
    ///   - conceptId: The concept identifier
    ///   - system: The code system URI
    /// - Returns: The preferred term, or nil if not found
    func lookupPreferredTerm(conceptId: String, system: String) async throws -> String? {
        ...
        do {
            let response: FHIRParameters = try await getJSON(url)
            let result = parseConceptFromParameters(response, conceptId: conceptId, system: system)
            return result?.pt ?? result?.fsn
        } catch {
            AppLog.debug(AppLog.network, "PT lookup failed for \(conceptId): \(error)")
            return nil
        }
    }
    ...
    private func getCachedOrFetchEditions() async throws -> [SNOMEDEdition] {
        if let cached = Self.cachedEditions,
           let cacheTime = Self.editionsCacheTime,
           Date().timeIntervalSince(cacheTime) < Self.editionsCacheTTL {
            return cached
        }
        let editions = try await fetchAllEditions()
        Self.cachedEditions = editions
        Self.editionsCacheTime = Date()
        return editions
    }
        ...
        for param in params.parameter ?? [] {
            ...
            case "property":
                if let parts = param.part {
                    var propCode: String?
                    var propValueString: String?
                    var propValueBoolean: Bool?
                    ...
                    if let code = propCode {
                        switch code {
                        case "inactive":
                            // Handle both boolean and string values for inactive property
                            if let boolValue = propValueBoolean {
                                active = !boolValue  // inactive=true means active=false
                            } else if let stringValue = propValueString {
    ...
    ///
    /// Attempts to parse the response as a FHIR OperationOutcome and extract the
    /// `diagnostics` or `details.text` field. Falls back to a generic message
    /// if parsing fails.
    ///
    /// - Parameters:
    ...
    private static func extractUserMessage(from data: Data, httpCode: Int) -> String {
        if let outcome = try? JSONDecoder().decode(FHIROperationOutcome.self, from: data),
           let message = outcome.userMessage, !message.isEmpty {
            return "HTTP \(httpCode): \(String(message.prefix(300)))"
        }
...
/// FHIR OperationOutcome resource returned for errors.
///
/// Contains structured error information from the server.
/// The `diagnostics` field is designed to be user-facing.
///
/// - SeeAlso: [FHIR OperationOutcome](https://hl7.org/fhir/R4/operationoutcome.html)
struct FHIROperationOutcome: Decodable {
    let resourceType: String?
    let issue: [Issue]?

    /// A single issue within the OperationOutcome.
    ...
    /// Extracts a user-facing message from the first issue.
    var userMessage: String? {
        guard let firstIssue = issue?.first else { return nil }
        return firstIssue.diagnostics ?? firstIssue.details?.text
    }
}
...
struct ValueSetExpansionResponse: Decodable {
    ...
    struct ExpansionContains: Decodable {
        ...
        let `extension`: [PropertyExtension]?
    }

    /// Extension for property values in ValueSet expansion (FHIR R5 style).
    ///
    /// Used to carry property values like `inactive` when requested via
    /// the `property` parameter on `ValueSet/$expand`.
    struct PropertyExtension: Decodable {
        let url: String?
        let `extension`: [PropertyExtensionPart]?
    }

    struct PropertyExtensionPart: Decodable {
        let url: String?
        let valueCode: String?
        let valueBoolean: Bool?
    }
...
// MARK: - ECL Evaluation Result

/// Result of evaluating an ECL expression.
///
/// Contains the total number of matching concepts on the server and the
/// subset returned within the requested limit.
struct ECLEvaluationResult {
    /// Total number of matching concepts (may be greater than `concepts.count`).
    let total: Int
    /// The matching concepts (up to the requested limit).
    let concepts: [ECLEvaluationConcept]
}

/// A concept returned from ECL evaluation.
///
/// Represents a single concept from a `ValueSet/$expand` response with
/// its SNOMED CT identifier, preferred term, and fully specified name.
struct ECLEvaluationConcept {
    /// The SNOMED CT concept ID.
    let code: String
    /// The preferred display term (PT).
    let display: String
    /// The fully specified name (FSN), if available from designations.
    let fsn: String?

    /// The semantic tag extracted from the FSN (e.g., "disorder", "finding").
    ///
    /// Extracted from the parenthesised suffix of the FSN:
    /// `"Diabetes mellitus (disorder)"` → `"disorder"`.
    var semanticTag: String? {
        guard let fsn else { return nil }
        guard let openParen = fsn.lastIndex(of: "("),
              let closeParen = fsn.lastIndex(of: ")"),
              openParen < closeParen else { return nil }
        let start = fsn.index(after: openParen)
        return String(fsn[start..<closeParen])
    }
}
