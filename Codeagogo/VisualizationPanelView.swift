...
struct WebViewRepresentable: NSViewRepresentable {
    ...
    class Coordinator: NSObject, WKScriptMessageHandler {
        func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
            ...
            // Copy to clipboard — no save panel needed
            if type == "clipboard" {
                guard let imageData = Data(base64Encoded: data),
                      let image = NSImage(data: imageData) else {
                    AppLog.error(AppLog.ui, "Failed to create image for clipboard")
                    return
                }
                let pb = NSPasteboard.general
                pb.clearContents()
                pb.writeObjects([image])
                AppLog.info(AppLog.ui, "Diagram copied to clipboard")
                return
            }
            ...
            savePanel.beginSheetModal(for: window) { response in
                ...
                do {
                    ...
                    AppLog.error(AppLog.ui, "Failed to save file: \(error)")
                }
            }
        }
