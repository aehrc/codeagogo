...
- **ecl-core 1.1.2**: Updated to ecl-core 1.1.2 with `removeRedundantParentheses` formatter option, canonical comparison, and historical association lookups.
- **ecl-editor 1.1.2**: Updated ECL Workbench bundle — inactive concept replacement quick fixes, shared language registration fix, toggle display terms (Shift+Alt+T), cleaner embedded appearance.
...
- **ECL Workbench** (`Control+Option+V`): Full-featured ECL editing environment with Monaco-based editor, syntax highlighting, FHIR-powered autocomplete, inline diagnostics, formatting, and live evaluation. Powered by `@aehrc/ecl-editor`.
- **ECL Format** (`Control+Option+E`): Toggle ECL expressions between pretty-printed and minified format with 9 configurable options. Powered by `@aehrc/ecl-core` via JavaScriptCore.
- **Open in Shrimp** (`Control+Option+H`): Open selected SNOMED CT concept in Shrimp browser.
- **Concept Diagrams**: SNOMED CT concept definition diagrams following the Diagramming specification, with SVG/PNG export. Locale-aware preferred terms and composite unit resolution (e.g., "mg/mL").
- **ECL Reference Panel**: 50 searchable knowledge articles covering all ECL operators, grouped by category.
- **Semantic Validation**: Background validation of concepts in ECL expressions with warnings for inactive or unknown concepts.
- **Update Checker**: Automatic checks for new releases (on launch + daily), with orange dot badge on menu bar icon, menu notification, and Settings banner.
- **Launch at Login**: Opt-in on first launch via welcome screen, configurable in Settings.
- **Homebrew Tap**: Install via `brew tap aehrc/codeagogo && brew install --cask codeagogo`.
- **Custom Menu Bar Icon**: Ontoserver-style cloud with network dots.
- **Configurable Hotkeys**: All six hotkeys fully customizable via Settings using a keystroke recorder.
- **Multi-Code-System Support**: Configurable additional code systems via Settings, queried via FHIR `CodeSystem/$lookup`.
